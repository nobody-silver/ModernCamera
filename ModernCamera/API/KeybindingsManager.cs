...
public static class KeybindingsManager
{
    ...
    public static void UpdateState(InputState inputState)
    {
        ...
        {
            Plugin.Logger.LogInfo("No inputs pressed");
            return;
        }

        foreach (var category in Categories.Values)
        {
            Plugin.Logger.LogInfo($"Checking category: {category.Name}");
            foreach (var keybinding in category.KeybindingMap.Values)
            {
                Plugin.Logger.LogInfo($"Checking keybinding: {keybinding.Description}");
                if (keybinding.IsPressed)
                {
                    Plugin.Logger.LogInfo($"Keybinding {keybinding.Description} is pressed");
